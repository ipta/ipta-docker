...
parser = argparse.ArgumentParser(
          description='run stochastic GW background analysis with enterprise')
...
parser.add_argument('-z', '--dmturnover',
                    dest='use_dmto', default=False,
                    action='store_true',
                    help="use a turnover spectrum in DMGP (instead of powerlaw)")
...
if args.use_dmto:
    dm_kappa = parameter.Uniform(0, 7)
    dm_lf0 = parameter.Uniform(-9, -7)
    dm_pl = utils.turnover(log10_A=dm_log10_A, gamma=dm_gamma,
                        lf0=dm_lf0, kappa=dm_kappa)
else:
